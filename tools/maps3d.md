...
| — | `maps3d-interior.mjs` | `interior-plan.png` with `--plan` | Runs Tier C over all 53,595 and prints the budget: 6.10 M instances, 114 a building, 39 kit pieces in the order they should be modelled. Asserts `rooms == walls + 1` on every level and exits non-zero if a room is ever sealed off. |
